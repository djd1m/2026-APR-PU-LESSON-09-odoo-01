...
The `capture_service.py` imports Redis inside a try/except block. This is correct for graceful degradation but means the module can be installed without `redis` Python package. In production, the FFmpeg worker requires Redis to function.
...
The `__manifest__.py` does not declare `"external_dependencies": {"python": ["redis"]}`. While the current code handles Redis absence gracefully, declaring it helps Odoo's dependency checker.
