...
> - MUST delegate to `/next`, `/go`, `/plan`, `/feature` via **Skill tool** — NEVER spawn raw Agent tools directly
...
> - FORBIDDEN: marking a feature as "done" unless `docs/features/<id>/review-report.md` exists
> - FORBIDDEN: skipping any of the 4 phases (PLAN → VALIDATE → IMPLEMENT → REVIEW)
>
> **KNOWN BUG (from LESSON-08):** LLM tends to skip Phases 1, 2, and 4 in autonomous mode,
> executing only Phase 3 (IMPLEMENT). This produces code without documentation, validation,
> or security review. The `/run` loop MUST verify after each `/go` call that ALL 4 phase
> artifacts exist before proceeding:
> - `docs/features/<id>/01_specification.md` (Phase 1)
> - `docs/features/<id>/validation-report.md` (Phase 2)
> - `docs/features/<id>/review-report.md` (Phase 4)
> If ANY artifact is missing, the feature is NOT complete — halt and report.
...
while features_remaining_in_scope:
    ...
    invoke Skill tool: /go feature_id          # MUST use Skill tool, NOT raw Agent
    verify completion:
      - tests green, code committed
      - docs/features/<id>/01_specification.md EXISTS (Phase 1 proof)
      - docs/features/<id>/validation-report.md EXISTS (Phase 2 proof)
      - docs/features/<id>/review-report.md EXISTS (Phase 4 proof)
    if ANY doc missing → feature is NOT done, halt with error
