...
## Phase 4 Enforcement (LESSON-08 Regression Prevention)

> **CRITICAL:** In project LESSON-08, Phase 4 (REVIEW) was skipped for ALL 13 features.
> The LLM "forgot" Phase 4 even after two explicit user reminders. This section exists
> to make that structurally impossible.

### Hard Rules

1. **A feature is NOT done without `review-report.md`.**
   `docs/features/<id>/review-report.md` MUST exist before `status: "done"` in roadmap.

2. **Phase 4 runs IMMEDIATELY after Phase 3 commit — not "later".**
   There is no "Phase 4 deferred" state. Phase 3 commit triggers Phase 4 automatically.

3. **`/run` and `/go` MUST verify artifacts before marking done.**
   Missing `review-report.md` = pipeline incomplete = feature stays `in_progress`.

4. **Phase 4 is NOT optional in AUTO mode.**
   AUTO mode skips *confirmations*, not *phases*. All 4 phases execute sequentially.

### Verification Checklist (for /run, /go, /feature)

```
AFTER Phase 3 commit, BEFORE marking feature done:
  [ ] docs/features/<id>/01_specification.md exists
  [ ] docs/features/<id>/validation-report.md exists
  [ ] docs/features/<id>/review-report.md exists
  [ ] No blocker-severity findings in review-report.md
  → ALL checked? → status: "done"
  → ANY missing? → HALT, do not proceed to next feature
```
...
| Executing only Phase 3 in `/run` | **LESSON-08 bug:** produced scaffold without docs/validation/review |
| Marking feature "done" without review-report.md | Phase 4 was "forgotten" twice in LESSON-08 |
| Spawning raw Agent tools instead of Skill tool | Bypasses pipeline rules; use `/go` → `/feature` via Skill tool |
