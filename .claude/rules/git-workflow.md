...
## Stop Hooks (auto-installed by `init` via `settings.json`)
...
### Hook Ordering Rule (LESSON-08 fix)

> **autopush.cjs MUST be the LAST entry in the Stop hooks array.**
> Claude Code executes Stop hooks sequentially. If autopush runs before
> autocommit-*, those commits won't be included in the push.
>
> Correct order in `.claude/settings.json`:
> 1. autocommit-roadmap.cjs
> 2. autocommit-insights.cjs
> 3. autocommit-plans.cjs
> 4. autopush.cjs ← ALWAYS LAST
